...
static int parseArgVec64(struct slice *b, struct parseState *st, u_int64_t *x)
{
    ...
    if (getU8(b, &sz) == -1)
        return -1;
    /* note: we ignore memory leaks - exit/doneWork are perfect GCs */
    vec = malloc(sz * sizeof vec[0]);
    if (sz && !vec)
        return -1;
    if (verbose)
	printf("argVec64 %llx - size %d\n", (unsigned long long)(u_long)vec, sz);
    for (i = 0; i < sz; i++) {
...
int
parseSysRec(struct sysRec *calls, int ncalls, struct slice *b, struct sysRec *x)
{
